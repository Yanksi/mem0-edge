...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; } body { margin: 0; min-height: 100vh; } button, input, select { font: inherit; }
    .shell { display: grid; grid-template-columns: 208px minmax(0, 1fr); min-height: 100vh; }
    .rail { display: grid; align-content: space-between; padding: 22px 14px; color: #d9e5e1; background: #17251f; }
    ...
    .workspace { padding: 28px clamp(20px, 4vw, 52px); overflow: hidden; } .topbar { display: flex; align-items: end; justify-content: space-between; gap: 18px; padding-bottom: 26px; border-bottom: 1px solid #d5dcda; }
    ...
    .memory-list { border-top: 1px solid #d5dcda; } .memory-row { width: 100%; display: grid; gap: 5px; padding: 15px 0; border: 0; border-bottom: 1px solid #d5dcda; background: transparent; text-align: left; cursor: pointer; } .memory-row:hover { background: #edf4f1; }
    ...
    .detail { margin-top: 22px; padding: 16px; border-left: 3px solid #65d6ad; background: #eaf2ee; white-space: pre-wrap; } .detail[hidden] { display: none; } .detail dl { display: grid; grid-template-columns: 110px 1fr; gap: 8px; margin: 14px 0 0; font-size: 13px; } .detail dt { color: #63716c; } .detail dd { margin: 0; overflow-wrap: anywhere; }
    ...
    @media (max-width: 720px) { .shell { grid-template-columns: 1fr; } .rail { grid-template-columns: auto 1fr auto; align-items: center; gap: 10px; padding: 10px; } .brand { padding: 0; } .nav { grid-auto-flow: column; justify-content: start; } .nav button { width: auto; } .nav-mark { display: none; } .logout { width: auto; } .workspace { padding: 20px; } .topbar { display: grid; align-items: stretch; } .search-form { display: grid; } }
  </style>
</head>
<body>
  <div class="shell">
    ...
    <main class="workspace">
      ...
      <section class="view" id="view-graph"><div class="section-head"><h2>Memory graph</h2><span id="graph-status" class="muted"></span></div><div class="graph" id="graph"></div></section>
      <aside class="detail" id="detail" hidden></aside>
    </main>
  </div>
  <script>
    const state = { userId: '', users: [], offset: 0, nextOffset: null, memories: [] };
    const labels = { search: ['Search memory', 'Semantic recall across a selected user profile.'], memories: ['All memories', 'Browse every active memory, newest first.'], graph: ['Memory graph', 'Explore entities and the relationships inferred from stored memories.'] };
    const select = document.getElementById('user-select'); const detail = document.getElementById('detail');
